...
filename = 'SirCovid.csv'
df = pd.read_csv(filename)
#convert dataframe to matrix
conv_arr1= df['Confirmed'].values
conv_arr2= df['Recovered'].values
conv_arr3 = df['Deaths'].values

arr1 = conv_arr1.ravel()
X_cml = np.array(arr1[45:530], dtype=np.float64)

arr2 = conv_arr2.ravel()
recovered = np.array(arr2[45:530], dtype=np.float64)

arr3 = conv_arr3.ravel()
death = np.array(arr3[45:530], dtype=np.float64)
...
clf_gamma = Ridge(alpha=0.0001675, copy_X=True, fit_intercept=False, max_iter=None,normalize=True, random_state=None, solver='auto', tol=1e-08).fit(x_gamma, y_gamma)
